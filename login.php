...
.app-wrapper { display: flex; width: 100%; min-height: 100vh; flex-direction: row; }

.hero-section {
    width: 42%; position: relative; background-color: #000; overflow: hidden;
...
.hero-section::after {
    ...
    background: linear-gradient(180deg, rgba(15,23,42,0.1) 0%, rgba(15,23,42,0.95) 100%); z-index: 2;
}

.slideshow, .slide { position: absolute; inset: 0; width: 100%; height: 100%; }
.slide { object-fit: cover; opacity: 0; animation: crossfade 18s infinite ease-in-out; }
...
.hero-content h2 { font-size: clamp(2rem, 3vw, 3rem); font-weight: 700; margin-bottom: 16px; }
...
.form-section {
    width: 58%; display: flex; justify-content: center; align-items: center;
    padding: clamp(24px, 5vw, 48px); background: var(--bg-color); overflow-y: auto;
}

.form-container {
    width: 100%; max-width: 520px; background: var(--surface-color);
    padding: clamp(24px, 4vw, 48px); border-radius: 24px;
    box-shadow: 0 20px 40px rgba(15,23,42,0.04); border: 1px solid var(--border-color);
}

.brand-logo { display: flex; align-items: center; gap: 12px; font-size: 1.5rem; font-weight: 700; color: var(--text-main); text-decoration: none; margin-bottom: 32px; }
.brand-logo svg { width: 32px; height: 32px; fill: var(--brand-primary); }

.auth-tabs { display: flex; background: var(--bg-color); padding: 6px; border-radius: 16px; margin-bottom: 32px; }
.auth-tab { flex: 1; text-align: center; padding: 12px; font-weight: 600; color: var(--text-muted); cursor: pointer; border-radius: 12px; transition: 0.3s; }
...
.form-header h3 { font-size: 1.5rem; font-weight: 700; margin-bottom: 8px; }
...
.input-field {
    width: 100%; padding: 24px 16px 8px; border: 1.5px solid var(--border-color);
    border-radius: 14px; background: var(--surface-color); font-size: 1rem; font-family: inherit; transition: 0.2s;
}
.input-field:focus { outline: none; border-color: var(--brand-primary); box-shadow: 0 0 0 4px rgba(79,70,229,0.1); }
.input-label { position: absolute; left: 16px; top: 50%; transform: translateY(-50%); color: var(--text-muted); pointer-events: none; transition: 0.2s; }
...
.pref-option input { position: absolute; opacity: 0; width: 100%; height: 100%; cursor: pointer; z-index: 2; }
.pref-option label { display: block; text-align: center; padding: 14px 10px; border: 1.5px solid var(--border-color); border-radius: 12px; font-weight: 600; color: var(--text-muted); transition: 0.2s; position: relative; z-index: 1; }
.pref-option input:checked + label { border-color: var(--brand-primary); background: rgba(79,70,229,0.04); color: var(--brand-primary); }

.pwd-toggle { position: absolute; right: 16px; top: 50%; transform: translateY(-50%); background: none; border: none; color: var(--text-muted); cursor: pointer; }
.btn-primary { width: 100%; padding: 16px; background: var(--brand-primary); color: #FFF; border: none; border-radius: 14px; font-size: 1rem; font-weight: 600; cursor: pointer; transition: 0.3s; margin-top: 8px; }
...
.checkbox-container { display: flex; align-items: center; gap: 10px; cursor: pointer; color: var(--text-muted); }

.alert-message { margin-top: 16px; padding: 14px; border-radius: 10px; font-weight: 500; display: none; text-align: center; }
...
@media (max-width: 768px) {
    ...
    .hero-section { width: 100%; height: 32vh; padding: 32px 24px; justify-content: center; }
    .form-section { width: 100%; min-height: 68vh; padding: 24px 16px; align-items: flex-start; }
    .form-container { margin-top: -30px; z-index: 10; padding: 28px 20px; }
...
async function handleAuth(e, action) {
    ...
    if(action === 'login') {
        ...
        formData.append('password', document.getElementById('regPwd').value);
    }

    try {
        
        const res = await fetch('config.php', { method: 'POST', body: formData });
        const data = await res.json();
        
        showAlert(alertId, data.message, data.status);
        if (data.status === 'success') {
            if(action === 'login') setTimeout(() => window.location.href = 'dashboard.php', 1500);
            if(action === 'signup') { e.target.reset(); setTimeout(() => switchTab('login'), 1500); }
        }
    } catch (err) {
        showAlert(alertId, 'Connection error.', 'error');
    } finally {
        btn.disabled = false; btn.innerHTML = action === 'login' ? 'Sign In' : 'Create Account';
    }
}
</script>
</body>
