                ...
                <div class="grid grid-cols-1 lg:grid-cols-2 gap-4 mb-8">
                    <div class="card flex flex-col sm:flex-row gap-4 items-start sm:items-center">
                        ...
                        <div class="flex gap-2 w-full sm:w-auto mt-2 sm:mt-0">
                            <button class="flex-1 sm:flex-none px-4 py-2 bg-slate-100 hover:bg-slate-200 dark:bg-slate-800 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-300 rounded-lg text-sm font-semibold transition-colors" onclick="openModal('view-seeker-modal')">View</button>
                            <button class="flex-1 sm:flex-none px-4 py-2 bg-brand-primary hover:bg-brand-hover text-white rounded-lg text-sm font-semibold transition-colors shadow-sm" onclick="acceptSeeker(this)">Accept</button>
                        </div>
                    </div>
                    <div class="card flex flex-col sm:flex-row gap-4 items-start sm:items-center">
...
<div id="add-property-modal" class="fixed inset-0 bg-black/60 backdrop-blur-sm z-[100] hidden items-center justify-center modal-overlay">
    <div class="bg-white dark:bg-slate-900 rounded-2xl w-full max-w-lg shadow-2xl border border-slate-200 dark:border-slate-800 modal-content mx-4 overflow-hidden flex flex-col max-h-[90vh]">
        ...
        <div class="p-6 overflow-y-auto">
            <form onsubmit="event.preventDefault(); submitForm('add-property-modal', 'Property added successfully!');" class="space-y-4">
                ...
                <div class="pt-4 flex gap-3">
                    ...
                    <button type="submit" class="flex-1 btn-primary">Save Property</button>
                </div>
            </form>
        </div>
    </div>
...
<div id="edit-property-modal" class="fixed inset-0 bg-black/60 backdrop-blur-sm z-[100] hidden items-center justify-center modal-overlay">
    <div class="bg-white dark:bg-slate-900 rounded-2xl w-full max-w-lg shadow-2xl border border-slate-200 dark:border-slate-800 modal-content mx-4 overflow-hidden flex flex-col max-h-[90vh]">
        <div class="px-6 py-4 border-b border-slate-200 dark:border-slate-800 flex justify-between items-center bg-slate-50 dark:bg-slate-900/50">
            <h3 class="text-xl font-bold text-slate-900 dark:text-white flex items-center gap-2">
                <span>✏️</span> Edit Property Details
            </h3>
        ...
        <div class="p-6 overflow-y-auto">
            <form onsubmit="event.preventDefault(); submitForm('edit-property-modal', 'Property updated successfully!');" class="space-y-4">
                ...
                <div class="pt-4 flex gap-3">
                    ...
                    <button type="submit" class="flex-1 btn-primary">Update Details</button>
                </div>
            </form>
        </div>
    </div>
    ...
    <div class="bg-white dark:bg-slate-900 rounded-2xl w-full max-w-sm shadow-2xl border border-slate-200 dark:border-slate-800 modal-content mx-4 overflow-hidden">
        <div class="p-6 text-center">
            ...
            <div class="flex gap-3">
                <button onclick="closeModal('view-seeker-modal')" class="flex-1 btn-secondary">Close</button>
                <button onclick="closeModal('view-seeker-modal'); showToast('Message sent to Rahul!');" class="flex-1 btn-primary bg-indigo-600">Message</button>
            </div>
        </div>
    </div>
</div>
...
<script>
    ...
    function submitForm(modalId, successMessage) {
        closeModal(modalId);
        showToast(successMessage);
    }

    // Handle Accept Seeker logic
    function acceptSeeker(btnElement) {
        btnElement.innerText = "Accepted ✓";
        btnElement.classList.remove('bg-brand-primary', 'hover:bg-brand-hover');
        btnElement.classList.add('bg-green-500', 'hover:bg-green-600', 'pointer-events-none');
